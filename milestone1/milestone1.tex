...
\begin{center}
\Large\textbf{Physics-Informed Machine Learning for Regional Geomagnetic Disturbance Forecasting}\\
\end{center}
...
\noindent Regional $|dB/dt|$ forecasting is structured around two supervised learning tasks:

\begin{table}[h]
\centering
\begin{tabular}{|l|p{5cm}|l|p{4cm}|}
\hline
\textbf{Task} & \textbf{Question} & \textbf{Lead Time} & \textbf{Target Stations} \\ \hline
\textbf{Classification} & Will $\|dB/dt\|$ exceed a hazard threshold at a SuperMAG station? (yes/no) & 60 minutes & ABK, BJN, TRO (Scandinavian auroral zone) \\ \hline
\textbf{Regression} \textit{(extension)} & How intense will $\|dB/dt\|$ be at a target station at time $t + 60$? & 60 minutes & ABK, BJN, TRO \\ \hline
\end{tabular}
\end{table}

\noindent \textbf{Classification Task} (\textit{Primary})
...
\textbf{Secondary} & \textbf{LSTM} & Sequence classifier; tests whether learned temporal structure adds skill beyond LightGBM's engineered lag features \\ \hline
\end{tabular}
\end{table}

\noindent \textbf{Regression Task} (\textit{Extension})
...
\hline
& \textbf{Model} & \textbf{Notes} \\ \hline
Baseline & Persistence & Predict $\|dB/dt\|_{t+60} = \|dB/dt\|_t$ \\ \hline
Baseline & Climatology mean & Predict training-set mean $\|dB/dt\|$; denominator for skill scores \\ \hline
\textbf{Primary} & \textbf{LSTM} & Sequence-to-scalar regression; same architecture as classification LSTM with output head swapped to predict continuous $\|dB/dt\|$ intensity \\ \hline
\end{tabular}
\end{table}
...
\begin{tabular}{|l|l|p{8cm}|}
\hline
\textbf{Model} & \textbf{Role} & \textbf{Key Justification} \\ \hline
Persistence & Baseline & Minimum operational bar; any useful model must exceed it \\ \hline
Climatology & Baseline & Denominator for Brier Skill Score \\ \hline
Logistic Regression & Linear reference & Establishes linear ceiling; interpretable coefficient view \\ \hline
LightGBM & \textbf{Primary model} & Handles imbalance, missing data, and tabular heterogeneity natively; SHAP-compatible \\ \hline
LSTM & Optional extension & Tests whether learned temporal structure adds skill beyond lag features \\ \hline
\end{tabular}
\caption{Model Roles and Justifications}
\end{table}
...
\noindent Calibration will be evaluated with a reliability diagram and expected calibration error (ECE). The model will also be evaluated stratified by storm intensity (quiet, moderate, intense) using GEM substorm event dates from SuperMAG where available. \\

\noindent The project is considered successful if the primary LightGBM model consistently outperforms both baseline models on the held-out test set on HSS and BSS, and produces a reliability diagram indicating that forecast probabilities are well-calibrated. Reaching the LSTM extension is a secondary goal that will be pursued if the primary model pipeline is complete and stable before the end of the quarter.
