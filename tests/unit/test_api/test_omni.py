...
# Repository root: tests/unit/test_api/ -> ../../..
...
class TestAuditLeakage(unittest.TestCase):
    def test_clean_sequences_pass(self):
        from swmi.sequences.builder import audit_leakage

        # One sample, 2 one-minute feature steps; T_last + 60 min = target
        feat_ts = np.array(
            [
                [
                    np.datetime64("2015-06-01T12:00"),
                    np.datetime64("2015-06-01T12:01"),
                ]
            ],
            dtype="datetime64[ns]",
        )
        target_ts = np.array([np.datetime64("2015-06-01T13:01")], dtype="datetime64[ns]")
        audit_leakage(feat_ts, target_ts, forecast_horizon_min=60)
    ...
    def test_misaligned_target_raises(self):
        ...
        target_ts = np.array([np.datetime64("2015-06-01T12:30")], dtype="datetime64[ns]")
        with self.assertRaises(ValueError):
            audit_leakage(feat_ts, target_ts, forecast_horizon_min=60)
